% Thomas C. Day
% Sweep an overlap threshold. I will use a series of overlap thresholds,
% and simulate N clusters at each. Measure mean and error in cluster size.
% Compare to experiment. Choose the threshold that is closest.

%% Inputs:
folder = 'E:\My Drive\01_Data\ProjectPloidy\202312\PA_2N_overlap\';
cd(folder);
diam0 = 2 .* ((3*(61.5)./(4*pi*1.24)).^(1/3)); % PA lines
diam0 = 2 .* ((3*(80.9)./(4*pi*1.26)).^(1/3)); % PM lines
ClusterRadiusReal = 53.7;

V               = [128.1]; %4/3*pi*(2.5^3);         % this is the volume of every cell
N               = 50;                                % number of clusters to generate at each parameter value
numGens         = 20;                               % number of generations of cell division in a group, here i pick a high value
AR              = [1.34];                           % aspect ratio value
...
distance_thresh = 1.1672*diam/diam0;                % minimum distance (um) separating bud scars, this grows proportionately to cell diameter
new_bud_prob    = .8;                               % probability that the first cell will bud near the pole
check_overlap   = 1;                                % do we check the overlap?
% overlap_thresh  = [12,13];
overlap_thresh  = linspace(overlap_thresh(1), overlap_thresh(2), 5) .* ((diam/diam0).^2) * 1e3; % threshold of total overlaps
% overlap_thresh  = 1.7e3 .* ((diam/diam0).^2);
% overlap_thresh = 2040;
% overlap_thresh = 637;
% overlap_thresh = 17365;
% overlap_thresh = 1065;

% Force input parameters:
include_forces  = 0; % do we include forces for rearrangements?
...
MeanRad = zeros(length(overlap_thresh),1);
ErrRad  = MeanRad;
for tt = 1:length(overlap_thresh)
    cells_sim = cell(N,1);
    aspRat    = AR;
    theta     = THETA;
    filename = ['thomas-sim','_AspRat=',num2str(aspRat,'%1.1f'),...
                '_CellDiam=',num2str(diam,3),...
                '_pole-theta=',num2str(rad2deg(pole_theta),3),'_theta=',num2str(rad2deg(theta),3),'_dtheta=',num2str(rad2deg(thetaVariance),3),...
                '_back-prob=',num2str(new_bud_prob,3),'_overlap-thresh=',num2str(overlap_thresh(tt)),...
                '_N=',num2str(N)];
    ...
    for ii = 1:N
        ...
        [cell_list] = ELYES_SIM(diam, err_diam, aspRat, err_AR, pole_theta, theta, thetaVariance, distance_thresh, new_bud_prob, numGens, NEIGHBOR_THRESH, check_overlap, overlap_thresh(tt));
%         [~,U] = CHECK_OVERLAPS2(cell_list, overlap_thresh(tt))
        ...
        % Save to voro file, if indicated:
        if voro_save == 1
    ...
    % Find mean cluster radius:
    for ii = 1:N
        centers   = [cells_sim{ii}.Center];
        COM       = mean(centers, 2);
        rCOM      = centers - COM;
        rCOM      = rCOM + (diam/2)*rCOM./vecnorm(rCOM);
        [~,VolCluster]     = convhull(rCOM');
        RAD(ii)   = ((3/4) * (1/pi) * VolCluster)^(1/3);
    end

    % Print mean group size:
    fprintf(['Mean group radius at fracture: ',num2str(mean(RAD)),'\n']);
    
    % Save radius to array:
    MeanRad(tt) = mean(RAD);
    ErrRad(tt)  = std(RAD);
    
    % Save data as a mat file:
    foldername = ['OverlapThresh=',num2str(overlap_thresh,'%04.f'),'\'];
    mkdir(foldername);
    save([foldername,'\','4N_Results.mat']);
...
line([overlap_thresh(1), overlap_thresh(end)],[ClusterRadiusReal, ClusterRadiusReal], 'linestyle','--','linewidth',1,'color','k');
errorbar(overlap_thresh, MeanRad, ErrRad, 'k.','linewidth',1,'markersize',8,'capsize',8);
xlabel('Overlap_thresh');
...
function [flag, U] = CHECK_OVERLAPS2(cell_list, overlap_thresh)
