...
#@ Boolean (label = "Use non-cluster region as background (otherwise use non-detected-cell region)", value = true) non_cluster_as_background  // non-cell region where cells are detected cells
...
print_and_save_log("Input directory for cluster ROIs: " + input1);
print_and_save_log("Input directory for cell ROIs: " + input2);
...
print_and_save_log("Use non-cluster region as background (otherwise use non-detected-cell region): " + tf(non_cluster_as_background));
print_and_save_log("Use batch mode 'hide': " + tf(batch_mode_hide));
print_and_save_log("Area unit: um^2");
print_and_save_log("");

// Parameters
se = 10;  // px, for cells on edges, cellpose can produce masks very close to edges but not really on edges, thus exclude cells/masks that are within this distance from edges
...
function processFolder(input, p) {
	...
	for (i = 0; i < list.length; i++) {
		...
		if (endsWith(file, suffix)) {
			...
			in_cluster_ROIs_zip = sample + "_cluster_ROIs.zip"; 
...
function processFile(input, output, file) {
	...
	// Load cell ROIs
	roiManager("Open", input2 + File.separator + in_ROIs_zip);
	// Report cell number
	print_and_save_log("Cell number: " + roiManager("count"));
	// Rename ROIs
	rename_rois();
	
	// Remove cell ROIs on image edges
	rn = roiManager("count");
	for (r = rn-1; r >= 0; r--) {
		roiManager("select", r);
		getSelectionBounds(x, y, w, h);
		if ( (x<=0+se) || (y<=0+se) || (x+w>=getWidth()-se) || (y+h>=getHeight()-se) ) roiManager("delete");
	}
	clear_selection();
	// Report cell number
	print_and_save_log("Cell number after excluding cells on image edges: " + roiManager("count"));
	// Rename ROIs
	rename_rois();
	// Save cell ROIs
	roiManager("Deselect");
	roiManager("Save", input2 + File.separator + out_ROIs_eoe_zip);
	...
	for (c = 0; c < measure_channels.length; c++) {
		...
		for (m = 0; m < metrics.length; m++) {
			...
			if (background_subtraction && (isin(metric, newArray("Mean", "RawIntDen")))) {
				...
				if (non_cluster_as_background) roiManager("Open", input1 + File.separator + in_cluster_ROIs_zip);  // cluster ROIs
				else roiManager("Open", input2 + File.separator + in_ROIs_zip);  // cell ROIs without excluding on edges
				...
				if (!report_background_area) {
					...
					background_area_fraction = round(getValue("Area raw") / (image_width*image_height) * 100);
				...
				roiManager("deselect"); roiManager("delete");
				roiManager("Open", input2 + File.separator + out_ROIs_eoe_zip);
			}
			// Measure
			rn = roiManager("count");
			for (r = 0; r < rn; r++) {
