...
for (condition in c("PM", "PA")) {
  ...
  result <- t.test(Mean_volume ~ 1, data = t200s, mu = t0)
  print(result)
}
# Cell AR
for (condition in c("PM", "PA")) {
  ...
  t200s <- data_summary %>%
    dplyr::filter(Condition == condition & EvoTime == "t200")
  result <- t.test(Mean_AR ~ 1, data = t200s, mu = t0)
  print(result)
}
```
...
df <- data_summary %>%
  dplyr::filter(Condition == condition & EvoTime != "t0")
result <- aov(Mean_volume ~ EvoTime, data = df)
summary(result) %>% print()
...
df <- data_summary %>%
  dplyr::filter(Condition == condition & EvoTime != "t0")
result <- aov(Mean_AR ~ EvoTime, data = df)
summary(result) %>% print()
...
result <- lm(Mean_AR ~ EvoTime2, data = df)
summary(result) %>% print()
```
